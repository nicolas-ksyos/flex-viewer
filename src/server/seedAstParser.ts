...
export function getSeedRelativePath(
...
export function hasErrorDiagnostics(diagnostics: SeedDiagnostic[]): boolean {
	return diagnostics.some((d) => !d.code || !d.code.startsWith("WARN"));
}
...
function evaluate(
	...
	if (ts.isArrayLiteralExpression(node)) {
		return node.elements.map((el) =>
			evaluate(el, checker, variables, fileSeed, uuidCounter),
		);
	}
	...
	if (ts.isBinaryExpression(node)) {
		if (node.operatorToken.kind === ts.SyntaxKind.QuestionQuestionToken) {
			...
			return (
				left ?? evaluate(node.right, checker, variables, fileSeed, uuidCounter)
			);
		}
...
export function parseSeedFile(
	...
	const knexServiceIdToMeta = new Map<
		string,
		{ name: string; code: string | null }
	>();
	...
	function processCallExpression(
		...
		if (
			...
			callExpr.expression.text === "createService" &&
			callExpr.arguments.length > 0
		) {
			const argVal = evaluate(
			...
			if (argVal && varName) {
				const serviceId = deterministicUuid(
					`${fileSeed}:createService:${varName}`,
				);
		...
		if (ts.isPropertyAccessExpression(callExpr.expression)) {
			...
			if (wf && methodName === "createStep" && callExpr.arguments.length > 0) {
				...
				if (stepData) {
					...
					if (result) {
						...
						if (result.stepActivities)
							wf.stepActivities.push(...result.stepActivities);
						if (result.pendingTransitions)
							wf.pendingTransitions.push(...result.pendingTransitions);
					}
				}
				return true;
			}
			...
			if (wf && methodName === "generateTransitions") {
				...
				if (callExpr.arguments.length > 0) {
					const extraTransitions = evaluate(
						callExpr.arguments[0],
						checker,
						variables,
						fileSeed,
							...
							if (et && typeof et === "object" && et.fromStep && et.toStep) {
								...
								if (fromId) {
									wf.pendingTransitions.push({
										fromStepId: fromId,
										toStepRef: et.toStep,
										onlyIfOutputEquals: et.onlyIfOutputEquals
											? String(et.onlyIfOutputEquals)
											: null,
										synchronous: false,
						...
						args.length >= 2
							? String(
									evaluate(
										...
										deterministicUuid(
											`${fileSeed}:service:${workflows.length}`,
										),
								)
		...
		for (const pt of wf.pendingTransitions) {
			...
			transitions.push({
				...
				id: deterministicUuid(
					`${fileSeed}:transition:${pt.fromStepId}:${toStepId}:${pt.synchronous}`,
				),
...
function buildActivity(
	...
	return {
		...
		id: String(
			data.id ??
				deterministicUuid(`${fileSeed}:activity:${uuidCounter.value++}`),
		),
...
function buildStep(
	...
	const stepId = String(
		data.id ?? deterministicUuid(`${fileSeed}:step:${uuidCounter.value++}`),
	);
	const blockId = deterministicUuid(`${fileSeed}:block:${blockName}`);

	const step: WorkflowStep = {
		allowedPerformer: data.allowedPerformer
			? String(data.allowedPerformer)
			: null,

...
export function useSeeds(clientSafePath: string | undefined) {
    ...
    const fetchSeeds = useCallback(async () => {
        if (!clientSafePath) { setSeeds([]); return; }
        setLoading(true);
        try {
    ...
    const startWatching = useCallback(async (seedFileName: string) => {
    ...
    const parseSeed = useCallback(async (seedFileName: string): Promise<SeedParseResult | null> => {
        try {
            const res = await fetch(`/api/seeds/${encodeURIComponent(seedFileName)}/parse`);
